...
def SFC_initial_formed(self, request_delay, request_reliability, VNFTyprList, VNFRequestCPU, VNFRequestMemory):
    ...
    while len(foundSFCList) < K:
        foundSFCList.append(find_a_SFC(request_delay, request_reliability, VNFTyprList, VNFRequestCPU, VNFRequestMemory))
    #step2, 判断每条SFC加入后是否会引起节点过载，并预测过载程度
    # 存放每条SFC的评分
    scoreList = None
    maxScore = 0  # 最高评分
    maxScoreSFCId = None  # 拥有最高评分的SFC的id
    for i in K:#挨个分析每一条SFC
        analysisSFC = foundSFCList[i]
        ...
        scoreList.append(SFC_placement_plan_scoring(analysisSFC.getSFCId()))
        if maxScore < SFC_placement_plan_scoring(analysisSFC.getSFCId()):
            maxScore = SFC_placement_plan_scoring(analysisSFC.getSFCId())
            maxScoreSFCId = analysisSFC.getSFCId()
    #返回拥有最高得分的SFC，作为最初的SFC部署方案
    return maxScoreSFCId

#SFC部署方案的评分方法
def SFC_placement_plan_scoring(SFC_id):
    #SFC的得分
    score = 0
    for i in len(foundSFCList):
        if SFC_id == foundSFCList[i].getSFCId():
            score = -foundSFCList[i].getDelay(SFC_id)
        break
    return score
